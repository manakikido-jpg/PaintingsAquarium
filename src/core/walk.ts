 ...
 */
/**
 * 同じ列で重なったまま並んで歩く組を減らす。
 *
 * `range` は「体の幅の何倍まで近づいたら引き返すか」。
 * **広くすれば良くなるわけではない。** 早く反応するほど反転が増え、
 * 引き返した先でまた別の1匹に当たって、その場で往復する（渋滞になる）。
 *
 * 実測（50匹・8列・30秒・重なったまま並走した最長の時間）:
 *
 * | 範囲 | 最長 |
 * |---|---|
 * | 避けない | 30.00秒（最初から最後までずっと） |
 * | 0.5 | 2.08秒 |
 * | **0.7** | **2.30秒** |
 * | 0.9 | 4.23秒 |
 * | 1.1 | 7.12秒 |
 *
 * 0.5 は「重なった」とみなす境目そのものなので余裕が無い。0.7 を採る。
 */
export function separateWalkers(
  walkers: readonly Walker[],
  { range = 0.7 }: { range?: number } = {},
): Walker[] {
