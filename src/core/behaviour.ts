 ...
 * 縦の縮み幅。
 *
 * **±13% まで上げたが「羽ばたいて見えない」と言われた（2026-08-24）。正しい。**
 * この方法では絵全体が同じ割合で縮むので、翼だけでなく**頭も嘴も縮む**。
 * 人は頭の形の変化に先に気づくので、「羽ばたき」ではなく
 * 「絵が潰れている」に見える。
 *
 * 翼だけを動かすには絵を切り分けるしかないが、**この台紙は切れない**。
 * 測ったところ、一番すいている縦線でも**絵の高さの 29% を横切る**
 *（四足恐竜の足元には、まったく横切らない隙間があった）。
 * 嘴から翼の先まで1つの塊としてつながっているため。
 *
 * そこで、羽ばたきは**あきらめて滑空にした**。
 * 縮みは「生きている感じ」を出すためだけの ±5% に落としてある。
 * 本当に羽ばたかせるなら、台紙を**翼が胴から離れて見えるポーズ**で
 * 作り直すしかない（`docs/設計-恐竜の台紙.md`）。
 */
export const FLAP_SQUASH = 0.05
/**
 * 縦に縮めたぶん横へ広げる割合。
