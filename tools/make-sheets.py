...
#
# **ここを変えるときは、題のことだけ考えてはいけない（R-069）。**
# 0.7倍にすると白い余白が 52% → 73% に増え、塗る面積が減って早く終わる。
# 二度目のイベントでは、その余白がクレヨンの落書きで埋まった紙が出て、
# **落書きが絵とつながって種類が付かなくなった**（プテラノドンが飛ばなかった）。
# 見分け側は直したが（`largestRegion`）、次に縮めるときは
# **余白に落書きした紙**も作って通してから出すこと。
ART_SCALE = 0.7
