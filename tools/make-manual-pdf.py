#!/usr/bin/env python3
"""現場マニュアルの HTML から、会場に持っていく PDF を作る。

    python3 tools/make-manual-pdf.py

**原本は `docs/イベント運営マニュアル.md`。** 手順を変えるときは、
そちらを先に直してから、この道具で作り直すこと。

**台紙の発注は入っていない。** 現場でやるのは発注以外だけなので、
現場に渡す紙に入れると、やらない手順を読ませることになる（本人の指示）。

なぜこのスクリプトが要るか（`make-proposal-pdf.py` と同じ理由）:
...
SOURCE = ROOT / 'docs' / 'イベント運営マニュアル-現場用.html'
OUTPUT = ROOT / 'docs' / 'お絵かき-現場マニュアル.pdf'
CHROME = '/opt/pw-browsers/chromium-1194/chrome-linux/chrome'
FAMILIES = 'family=Zen+Kaku+Gothic+New:wght@400;500;700&family=JetBrains+Mono:wght@400;700'
...
def main() -> None:
    ...
    printable = ROOT / 'docs' / '.現場マニュアル-印刷用.html'
